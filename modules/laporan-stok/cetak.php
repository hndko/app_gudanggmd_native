...
// jika user sudah login, maka jalankan perintah untuk cetak
  ...
  if ($stok == 'Seluruh') {
                ...
                <table class="table table-bordered" width="100%" cellspacing="0">
                  <thead class="bg-secondary text-white text-center" style="border: 1px solid black;">
                    <tr>
                      ...
                      <th style="border: 1px solid black;">ID Barang</th>
    ...
    while ($data = mysqli_fetch_assoc($query)) {
      // tampilkan data
      $html .= '		<tr>
                      <td style="border: 1px solid black;" width="50" class="text-center" align="center">' . $no++ . '</td>
                      <td style="border: 1px solid black;" width="80" class="text-center">' . $data['id_barang'] . '</td>
                      <td style="border: 1px solid black;" width="200">' . $data['nama_barang'] . '</td>
                      <td style="border: 1px solid black;" width="140"class="text-center"align="center">' . $data['nama_jenis'] . '</td>';
      // mengecek data "stok"
      // jika data stok minim, tampilkan data dengan warna background
      if ($data['stok'] <= $data['stok_minimum']) {
        $html .= '		<td  style="border: 1px solid black;"width="70" class="text-center"align="center"
                        <span class="badge badge-warning">' . $data['stok'] . '</span>
                      </td>';
      }
      // jika data stok tidak minim, tampilkan data tanpa warna background
      else {
        $html .= '		<td  style="border: 1px solid black;"width="70" class="text-center"align="center">' . $data['stok'] . '</td>';
      }
      $html .= '			<td style="border: 1px solid black;" width="80" class="text-center"align="center">' . $data['nama_satuan'] . '</td>
								    </tr>';
    }
    $html .= '		</tbody>
                </table>
  ...
  // jika filter data stok "Minimum" dipilih, tampilkan laporan stok barang yang mencapai batas minimum
    ...
    while ($data = mysqli_fetch_assoc($query)) {
      // tampilkan data
      $html .= '		<tr>
                      <td style="border: 1px solid black;" width="50" class="text-center">' . $no++ . '</td>
                      <td style="border: 1px solid black;" width="80" class="text-center">' . $data['id_barang'] . '</td>
                      <td style="border: 1px solid black;" width="200">' . $data['nama_barang'] . '</td>
                      <td style="border: 1px solid black;" width="140">' . $data['nama_jenis'] . '</td>
                      <td style="border: 1px solid black;" width="70" class="text-right">' . $data['stok'] . '</td>
                      <td style="border: 1px solid black;" width="80">' . $data['nama_satuan'] . '</td>
                    </tr>';
    }
    $html .= '		</tbody>
                </table>
