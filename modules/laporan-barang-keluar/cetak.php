...
// jika user sudah login, maka jalankan perintah untuk cetak
  ...
  $html = '<!DOCTYPE html>
          ...
          <body class="text-dark">
            <div class="text-center">
              <h2>LAPORAN DATA BARANG KELUAR GUDANG GMD</h2>
              <span>Tanggal ' . $tanggal_awal . ' s.d. ' . $tanggal_akhir . '</span>
            </div>
            <hr>
            <div class="mt-4">
              <table class="table table-bordered" width="100%" cellspacing="0">
                <thead class="bg-secondary text-white text-center" style="border: 1px solid black;">
		    <tr>
                    <th style="border: 1px solid black;">No.</th>
                    <th style="border: 1px solid black;">ID Transaksi</th>
		    <th style="border: 1px solid black;">PIC</th>
                    <th style="border: 1px solid black;">Tanggal</th>
                    <th style="border: 1px solid black;">NO. Bon-Nama Barang</th>
  ...
  $query = mysqli_query($mysqli, "SELECT a.id_transaksi, a.tanggal, a.barang, a.jumlah, b.nama_barang, c.nama_satuan,a.pic, a.keterangan, a.inputby
  ...
  while ($data = mysqli_fetch_assoc($query)) {
    // tampilkan data
    $html .= '	<tr>
                    <td style="border: 1px solid black;" width="30" class="text-center" align="center">' . $no++ . '</td>
                    <td style="border: 1px solid black;" width="90" class="text-center">' . $data['id_transaksi'] . '</td>
		    <td style="border: 1px solid black;" width="50" class="text-center" align="center">' . $data['pic'] . '</td>
                    <td style="border: 1px solid black;" width="50" class="text-center">' . date('d-m-Y', strtotime($data['tanggal'])) . '</td>
                    <td style="border: 1px solid black;" width="200">' . $data['barang'] . ' - ' . $data['nama_barang'] . '</td>
                    <td style="border: 1px solid black;" width="30" class="text-center" align="center">' . number_format($data['jumlah'], 0, '', '.') . '</td>
                    <td style="border: 1px solid black;" width="40" align="center">' . $data['nama_satuan'] . '</td>
                    <td style="border: 1px solid black;" width="100">' . $data['keterangan'] . '</td>
                    <td style="border: 1px solid black;" width="70" class="text-center" align="center">' . $data['inputby'] . '</td>
                  </tr>';
  }
  $html .= '		</tbody>
              </table>
